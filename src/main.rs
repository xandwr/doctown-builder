...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n📊 Clustering Results:");
    println!("   • Embeddings generated: {}", cluster_result.embeddings_generated);
    println!("   • Clusters created: {}", cluster_result.clusters_created);
    println!("   • Similarity edges: {}", cluster_result.similarity_edges_added);

    // Show sample clusters
    println!("\n🗂️ Sample Clusters:");
    let clusters: Vec<_> = graph.nodes.values()
    ...
    let output_path = "docpack-graph.json";
    ...
    match pipeline::generate::generate_documentation(&graph, &gen_config).await {
        Ok(result) => {
            ...
            let doc_path = "docpack-documentation.json";
