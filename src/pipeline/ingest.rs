...
fn extract_zip_to_memory(zip_bytes: Vec<u8>) -> Result<HashMap<String, Vec<u8>>, Box<dyn std::error::Error>> {
...
fn should_process(filename: &str) -> bool {
    ...
    let extensions = [
        ...
        ".c", ".cpp", ".h", ".hpp", ".cs", ".rb", ".lua", ".sh", 
        ".bash", ".sql", ".yaml", ".yml", ".toml", ".json", ".md", 
        ".html", ".css", ".swift", ".dockerfile"
    ];
    
    extensions.iter().any(|ext| filename.to_lowercase().ends_with(ext))
}